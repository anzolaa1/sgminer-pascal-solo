...
static double get_work_blockdiff(const struct work *work)
{
  ...
  {
		uint8_t *bits = &work->pool->swork.diff;
		shift = bits[0];
		powdiff = (8 * (0x1d - 3)) - (8 * (shift - 3));;
       ...
       char *hashshow, bool resubmit, char *worktime)
{
  ...
  if (json_is_true(res) || (work->gbt && json_is_null(res)) || (work->pool->algorithm.type == ALGO_PASCAL && !json_is_null(res))) {
...
static void rebuild_nonce(struct work *work, uint32_t nonce)
{
  ...
  else if (work->pool->algorithm.type == ALGO_PASCAL) nonce_pos = 292;
